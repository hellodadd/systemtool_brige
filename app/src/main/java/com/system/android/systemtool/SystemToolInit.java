...
import static com.system.android.systemtool.SystemToolBridge.hkAllConstructors;
import static com.system.android.systemtool.SystemToolBridge.hkAllMethods;
...
/*package*/ final class SystemToolInit {
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			STool_MethodHk callback = new STool_MethodHk() {
				...
				protected void beforeHkedMethod(MethodHkParam param) throws Throwable {
					SystemToolBridge.closeFileBeforeFkNative();
				}

				@Override
				protected void afterHkedMethod(MethodHkParam param) throws Throwable {
					SystemToolBridge.reopenFileAfterFkNative();
				}
			};
		...
		findAndHkMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new STool_MethodHk() {
			...
			protected void beforeHkedMethod(MethodHkParam param) throws Throwable {
				...
				STool_PackageLoad.LoadPackageParam lpparam = new STool_PackageLoad.LoadPackageParam(SystemToolBridge.sLoadedPackageCallbacks);
	...
	/*package*/ static void hkResources() throws Throwable {
		...
		if (!SystemToolBridge.initXSToolResourcesNative()) {
	...
	private static SToolResources cloneToXSToolResources(STool_MethodHk.MethodHkParam param, String resDir) {
		...
		if (newRes.isFirstLoad()) {
			...
			STool_InitPackageRes.InitPackageResourcesParam resparam = new STool_InitPackageRes.InitPackageResourcesParam(SystemToolBridge.sInitPackageResourcesCallbacks);
	...
	private static void hookSystemToolInstaller(ClassLoader classLoader) {
		try {
			findAndHkMethod(INSTALLER_PACKAGE_NAME + ".SystemToolApp", classLoader, "getActiveSystemToolVersion",
					STool_MethodRplmt.returnConstant(SystemToolBridge.getSystemToolVersion()));

			findAndHkMethod(INSTALLER_PACKAGE_NAME + ".SystemToolApp", classLoader, "onCreate", new STool_MethodHk() {
				@Override
				protected void afterHkedMethod(MethodHkParam param) throws Throwable {
				...
				try {
					...
					if (SystemToolBridge.isZygote) {
						...
						if (moduleInstance instanceof ISystemToolHkLoadPackage)
							SystemToolBridge.hkLoadPackage(new ISystemToolHkLoadPackage.Wrapper((ISystemToolHkLoadPackage) moduleInstance));

						if (moduleInstance instanceof ISystemToolHkInitPackageResources)
							SystemToolBridge.hkInitPackageResources(new ISystemToolHkInitPackageResources.Wrapper((ISystemToolHkInitPackageResources) moduleInstance));
					} else {
