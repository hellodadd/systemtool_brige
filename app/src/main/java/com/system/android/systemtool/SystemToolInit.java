...
import com.system.android.systemtool.callbacks.STool_InitPackageResources;
...
import com.system.android.systemtool.services.BaseService;

import static com.system.android.systemtool.SystemToolBridge.hookAllConstructors;
import static com.system.android.systemtool.SystemToolBridge.hookAllMethods;
import static com.system.android.systemtool.SystemToolHelpers.callMethod;
import static com.system.android.systemtool.SystemToolHelpers.closeSilently;
import static com.system.android.systemtool.SystemToolHelpers.fileContains;
import static com.system.android.systemtool.SystemToolHelpers.getMethod;
...
import static com.system.android.systemtool.SystemToolHelpers.getBooleanField;
import static com.system.android.systemtool.SystemToolHelpers.getObjectField;
import static com.system.android.systemtool.SystemToolHelpers.getOverriddenMethods;
import static com.system.android.systemtool.SystemToolHelpers.getParameterIndexByType;
import static com.system.android.systemtool.SystemToolHelpers.setObjectField;
import static com.system.android.systemtool.SystemToolHelpers.setStaticBooleanField;
import static com.system.android.systemtool.SystemToolHelpers.setStaticLongField;
import static com.system.android.systemtool.SystemToolHelpers.setStaticObjectField;

/*package*/ final class SystemToolInit {
	private static final String TAG = SystemToolBridge.TAG;

	private static final boolean startsSystemServer = SystemToolBridge.startsSystemServer();
	private static final String startClassName = SystemToolBridge.getStartClassName();

	private static final String INSTALLER_PACKAGE_NAME = "com.system.android.systemtool.installer";
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			STool_MethodHk callback = new STool_MethodHk() {
				...
				protected void startGetMethod(MethodHkParam param) throws Throwable {
					SystemToolBridge.closeFilesBeforeForkNative();
				}

				@Override
				protected void endGetMethod(MethodHkParam param) throws Throwable {
					SystemToolBridge.reopenFilesAfterForkNative();
				}
			};
		...
		getMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new STool_MethodHk() {
			...
			protected void startGetMethod(MethodHkParam param) throws Throwable {
				...
				if (instrumentationName != null) {
					...
					SystemToolBridge.disableHooks = true;
					return;
				}
				...
				STool_LoadPackage.LoadPackageParam lpparam = new STool_LoadPackage.LoadPackageParam(SystemToolBridge.sLoadedPackageCallbacks);
				...
				if (reportedPackageName.equals(INSTALLER_PACKAGE_NAME))
					hookSystemToolInstaller(lpparam.classLoader);
			}
		});
			...
			getMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new STool_MethodHk() {
						...
						protected void startGetMethod(MethodHkParam param) throws Throwable {
							...
							lpparam.classLoader = SystemToolBridge.BOOTCLASSLOADER;
				...
				protected void endGetMethod(MethodHkParam param) throws Throwable {
					...
					getMethod("com.android.server.SystemServer", cl, "startBootstrapServices", new STool_MethodHk() {
						...
						protected void startGetMethod(MethodHkParam param) throws Throwable {
							...
							} catch (SystemToolHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", SystemToolBridge.BOOT_START_TIME);
		}
	...
	/*package*/ static void hookResources() throws Throwable {
		...
		if (!SystemToolBridge.initSToolResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			SystemToolBridge.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}
		...
		hookAllConstructors(TypedArray.class, new STool_MethodHk() {
			...
			protected void endGetMethod(MethodHkParam param) throws Throwable {
				...
				if (res instanceof SToolResources) {
					SystemToolBridge.setObjectClass(typedArray, SToolResources.SToolTypedArray.class);
				}
			}
		});

		// Replace system resources
        SToolResources systemRes = (SToolResources) SystemToolBridge.cloneToSubclass(Resources.getSystem(), SToolResources.class);
	...
	private static SToolResources cloneToXResources(STool_MethodHk.MethodHkParam param, String resDir) {
        ...
        SToolResources newRes = (SToolResources) SystemToolBridge.cloneToSubclass(result, SToolResources.class);
		newRes.initObject(resDir);

		// Invoke handleInitPackageResources().
		if (newRes.isFirstLoad()) {
			String packageName = newRes.getPackageName();
			STool_InitPackageResources.InitPackageResourcesParam resparam = new STool_InitPackageResources.InitPackageResourcesParam(SystemToolBridge.sInitPackageResourcesCallbacks);
	...
	private static void hookSystemToolInstaller(ClassLoader classLoader) {
		try {
			getMethod(INSTALLER_PACKAGE_NAME + ".SystemToolApp", classLoader, "getActiveSystemToolVersion",
					STool_MethodReplacement.returnConstant(SystemToolBridge.getSystemToolVersion()));

			getMethod(INSTALLER_PACKAGE_NAME + ".SystemToolApp", classLoader, "onCreate", new STool_MethodHk() {
				@Override
				protected void endGetMethod(MethodHkParam param) throws Throwable {
					...
					if (res.getIdentifier("installer_needs_update", "string", INSTALLER_PACKAGE_NAME) == 0) {
						...
						Log.e("SystemToolInstaller", "SystemTool Installer is outdated (resource string \"installer_needs_update\" is missing)");
						Toast.makeText(application, "Please update SystemTool Installer!", Toast.LENGTH_LONG).show();
					}
				}
			});
		} catch (Throwable t) { Log.e(TAG, "Could not hook SystemTool Installer", t); }
	}
	...
	/*package*/ static void loadModules() throws IOException {
		...
		ClassLoader topClassLoader = SystemToolBridge.BOOTCLASSLOADER;
		ClassLoader parent;
	 ...
	 * in <code>assets/systemtool_init</code>.
	 */
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(SystemToolBridge.class.getName(), topClassLoader) != null) {
			Log.e(TAG, "  Cannot load module:");
			Log.e(TAG, "  The SystemTool API classes are compiled into the module's APK.");
			Log.e(TAG, "  This may cause strange issues and must be fixed by the module developer.");
			Log.e(TAG, "  For details, see: http://api.systemtool.info/using.html");
			closeSilently(dexFile);
			return;
		}
		...
		try {
			...
			ZipEntry zipEntry = zipFile.getEntry("assets/systemtool_init");
			if (zipEntry == null) {
				Log.e(TAG, "  assets/systemtool_init not found in the APK");
				closeSilently(zipFile);
				return;
			}
			is = zipFile.getInputStream(zipEntry);
		} catch (IOException e) {
			Log.e(TAG, "  Cannot read assets/systemtool_init in the APK", e);
			closeSilently(zipFile);
			return;
		}

		ClassLoader mcl = new PathClassLoader(apk, SystemToolBridge.BOOTCLASSLOADER);
		...
		try {
			...
			while ((moduleClassName = moduleClassesReader.readLine()) != null) {
				...
				try {
					...
					if (!ISystemToolMod.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class doesn't implement any sub-interface of ISystemToolMod, skipping it");
						continue;
					} else if (disableResources && ISystemToolHkInitPackageResources.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class requires resource-related hooks (which are disabled), skipping it.");
						continue;
					}

					final Object moduleInstance = moduleClass.newInstance();
					if (SystemToolBridge.isZygote) {
						if (moduleInstance instanceof ISystemToolHkZygoteInit) {
							ISystemToolHkZygoteInit.StartupParam param = new ISystemToolHkZygoteInit.StartupParam();
							param.modulePath = apk;
							param.startsSystemServer = startsSystemServer;
							((ISystemToolHkZygoteInit) moduleInstance).initZygote(param);
						}

						if (moduleInstance instanceof ISystemToolHkLoadPackage)
							SystemToolBridge.hookLoadPackage(new ISystemToolHkLoadPackage.Wrapper((ISystemToolHkLoadPackage) moduleInstance));

						if (moduleInstance instanceof ISystemToolHkInitPackageResources)
							SystemToolBridge.hookInitPackageResources(new ISystemToolHkInitPackageResources.Wrapper((ISystemToolHkInitPackageResources) moduleInstance));
					} else {
						if (moduleInstance instanceof ISystemToolHkCmdInit) {
							ISystemToolHkCmdInit.StartupParam param = new ISystemToolHkCmdInit.StartupParam();
							param.modulePath = apk;
							param.startClassName = startClassName;
							((ISystemToolHkCmdInit) moduleInstance).initCmdApp(param);
						}
					}
				} catch (Throwable t) {
