...
import static com.system.android.systemtool.SystemToolHelpers.inputStreamToByteArray;
...
/*package*/ class DexCreator {
	...
	public static File getDefaultFile(String childClz) {
		return new File(DALVIK_CACHE, "systemtool_" + childClz.substring(childClz.lastIndexOf('.') + 1) + ".dex");
	}
	...
	public static File ensure(String clz, Class<?> realSuperClz, Class<?> topClz) throws IOException {
		...
		try {
			return ensure("systemtool.dummy." + clz + "SuperClass", realSuperClz);
		} catch (IOException e) {
